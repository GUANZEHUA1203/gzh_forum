...
import com.gzeh.forum.common.OSSConfig;

public abstract class OSSUnit {
	 private static final String SYSTEM_PATH ="config.dev.properties";
	/** 
    ...
    public static String uploadFile(InputStream fileContent, String remotePath,String fileName) throws Exception {  
        ...
        // 加载配置文件，初始化OSSClient  
        OSSConfig ossConfigure = new OSSConfig(SYSTEM_PATH);  
		...
		// 上传文件
		PutObjectResult result = ossClient.putObject(ossConfigure.getBucketName(), remoteFilePath + fileName,
		...
		// 关闭io流
		fileContent.close();
		if (null == result || Strings.isNullOrEmpty(result.getETag()))
			return null;
    ...
    public static void downloadFile(OSSConfig ossConfigure, String key, String filename)  
